...
for i in range(num_matrices):
    y_vals, x_vals = np.where(matrix_47[i] == 1)
    z_vals = np.full_like(y_vals, i)
...
for i in range(num_matrices):
    y_vals, x_vals = np.where(matrix_144[i] == 1)
    z_vals = np.full_like(y_vals, i)
    ax2.scatter(y_vals, x_vals, marker='o', color='red')

# Ajustar límites de los ejes y configurar el eje X hacia abajo
ax1.set_xlim(0, 96)  # Limitar el eje X de 0 a 96 para el evento 47
ax1.set_ylim(0, 96)  # Limitar el eje Z (cámaras del detector) de 0 a 48 para el evento 47
ax2.set_xlim(0, 96)  # Limitar el eje X de 0 a 96 para el evento 144
ax2.set_ylim(0, 96)  # Limitar el eje Z (cámaras del detector) de 0 a 48 para el evento 144

# Etiquetas y título de los gráficos
ax1.set_xlabel('Eje y [cm]', fontsize=20)
ax1.set_ylabel('Eje x [cm]', fontsize=20)
...
ax2.set_xlabel('Eje y [cm]', fontsize=20)
ax2.set_ylabel('Eje x [cm]', fontsize=20)
